...
SELECT
  ...
  case
    WHEN num_clicked_pay > 0 and num_sent_to_auth>0 then 'success'
    else 'failed' END as payment_status_in_stage,

  case
    WHEN num_clicked_pay > 0 and num_sent_to_auth>0 then 'successfuly_sent_to_pre_auth'
    WHEN num_clicked_pay > 0 and num_sent_to_auth=0 then 'clicked_pay_but_not_sent_to_pre_auth'
