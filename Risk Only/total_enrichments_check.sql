...
select count(*) from r_payment_events p left join enrich_email_age ea on p.email_raw = ea.email where  payment_id in ($[pid]) 
and ea.data is null;

--@WbResult request emailage
select count(*) from r_payment_events p left join enrich_email_age ea on (p.partner_specific #>> '{payment_request, email}') = ea.email where  payment_id in ($[pid]) 
and ea.data is null;


--@WbResult seller emailage
with seller_data as (select email_raw from r_partner_seller_snap_events where seller_id in (
select partner_seller_id from r_payment_events where payment_id in ($[pid])))
...
with buyer_data as (select  coalesce((simplex_session #>> '{ip}'), (partner_session #>> '{ip}')) as ip, address1_raw as address1
 from r_payment_events where payment_id in (($[pid])) ),
...
select partner_seller_id from r_payment_events where payment_id in ($[pid])) group by 1, 2,3),
mm_data as (select request_data ->> 'i' as mm_ip, request_data ->> 'shipAddr' as mm_add1, data from enrich_maxmind)
...
with buyer_data as (select payment_id, coalesce((simplex_session #>> '{ip}'), (partner_session #>> '{ip}')) ip from r_payment_events where payment_id in (($[pid])))
...
select partner_seller_id from r_payment_events where payment_id in ($[pid])) group by 1, 2)
...
with buyer_data as (select coalesce((simplex_session #>> '{ip}'), (partner_session #>> '{ip}')) ip, first_name_raw as first_name from r_payment_events where payment_id in (($[pid]))
...
select partner_seller_id from r_payment_events where payment_id in ($[pid])) and country_raw in ('US', 'CA') group by 1, 2, 3 )
