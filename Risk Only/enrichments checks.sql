...
select * from enrich_email_age where email in (select email_raw from r_payment_events where payment_id in ($[pid]));

--@WbResult request emailage
select * from enrich_email_age where email in (select partner_specific #>> '{payment_request, email}' from r_payment_events where payment_id in ($[pid]));
...
with buyer_data as (select * from r_payment_events where payment_id in (($[pid])))
